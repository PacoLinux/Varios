...
*/
public class IntelPeep extends RollBackBuffer implements Serializable
{
...
//	System.out.println("Peephole optimise");
        ...
        if(l .indexOf("mov") ==0||l.indexOf("vmov")==0)
            {
                ...
                if(largs[0].equals(largs[1])&& largs[0].length()>0)
                    {
                        return "; optimised away : "+l;
                    }
            }
        return l;
    }
    String [][]pairs= {{"jz","jnz"},{"jg","jng"},{"jl","jnl"},{"ja","jna"},{"jle","jg"},{"jge","jnge"}};
    String merge(String []a)
    {
        ...
        for(int j=0; j<2; j++)
            for(int i=0; i<pairs.length; i++)
                {
                    if(f.startsWith(pairs[i][0+j]+" ")&&s.startsWith("jmp ")&&t.startsWith(getdest(f)))
                        {
                            return pairs[i][1-j]+" "+getdest(s);
                        }
                }
        return "";
    }
    ...
    void mergetriple(int i)
    {
        String []a =
        {
            store.elementAt(i).toString(),store.elementAt(i+1).toString(),store.elementAt(i+2).toString()
        };
        String merged=merge(a);
        if(!merged.equals(""))
            {
                store.setElementAt(" "+merged,i);
                store.setElementAt("; removed redundant jumps "+a[0]+";"+a[1]+";",i+1);
            }
    }
    ...
    String getdest(String l)   // assume that we have a jump instruction look for the label
    {
        ...
        try
            {
                while(l.charAt(i)==' ')i++;
                while(i<l.length()&&(Character.isLetterOrDigit(l.charAt(i))||l.charAt(i)=='$'||l.charAt(i)=='_'))
                    {
                        lab=lab+l.charAt(i);
                        i++;
                    }
                if(lab.equals("NEAR") || lab.equals("near"))
                    {
                        lab="";
                        while(l.charAt(i)==' ')i++;
                        while(i<l.length()&&(Character.isLetterOrDigit(l.charAt(i))||l.charAt(i)=='$'||l.charAt(i)=='_'))
