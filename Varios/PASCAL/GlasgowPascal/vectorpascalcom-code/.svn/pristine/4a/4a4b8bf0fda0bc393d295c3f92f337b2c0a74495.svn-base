...
pattern ixmreg means[iXMM00|iXMM10|iXMM20|iXMM30|iXMM50|iXMM40|iXMM70|iXMM60];
...
pattern imreg means[iMM00|iMM10|iMM20|iMM30|iMM70|iMM60|iMM50|iMM40];
...
pattern xmregi324 means[XMM0I324|XMM3I324|XMM2I324|XMM1I324|XMM4I324|XMM5I324|XMM6I324];
...
pattern psxmreg means[ XMM3R324| XMM2R324| XMM1R324| XMM0R324|XMM5R324];
...
instruction  pattern OPPI16(nonlog op, xmreg16 rm,xmreg16 r1)
    	...
    	assembles['p'op 'w  '    r1 ',' rm];
instruction  pattern OPPI32(nonlog op, xmregi324 rm,xmregi324 r1)
        means[  (ref i32x4) r1 :=op( (i32x4) ^( r1), (i32x4)^(  rm))]
    	assembles['p'op 'd  '    r1 ',' rm];
instruction  pattern SOPPI32(nonlog op, ixmreg  rm,ixmreg r1)
        means[    r1 :=op(  ^( r1),  ^(  rm))]
    	assembles['p'op 'd  '    r1 ',' rm];
...
instruction pattern BVEXTEND64( nonaccreg r)
	means[ PUSH(mainSTACK,(int32 vector ( 4 ))EXTEND((int8 vector ( 4 ))^(mem(	 ^(r)))))]
	assembles['sub   rsp,16\n'
			  'push  rax\n'
			  'movsx eax,byte['r']; expand int8 vector to 'x'\n'
			  'mov   dword[rsp+8],eax\n'
			  'movsx eax,byte['r'+1]\n'
			  'mov   dword[rsp+4+8],eax\n'
			  'movsx eax,byte['r'+2]\n'
			  'mov   dword[rsp+8+8],eax\n'
			  'movsx eax,byte['r'+3]\n'
			  'mov   dword[rsp+12+8],eax\n'
			  'pop   rax\n'
			   ];
instruction pattern BVEXTEND32( nonaccreg r)
	means[ PUSH(mainSTACK,(int32 vector ( 4 ))EXTEND((int8 vector ( 4 ))^(mem(	 ^(r)))))]
	assembles['sub   esp,16\n'
			  'push  eax\n'
			  'movsx eax,byte['r']; expand int8 vector to 'x'\n'
			  'mov   dword[esp+4],eax\n'
			  'movsx eax,byte['r'+1]\n'
			  ...
			  'movsx eax,byte['r'+3]\n'
			  'mov   dword[esp+12+4],eax\n'
			   'pop eax'];	          
instruction pattern POPPI2PS64(psxmreg r)
		means [r:=(ieee32 vector ( 4 ))FLOAT((int32 vector ( 4 ))POP(mainSTACK))]
		assembles['sub rsp,16\n CVTPI2PS 'r', [rsp]\n SHUFPS 'r','r', 044H; move low pair to high\n'
		           ' CVTPI2PS 'r',[rsp+8]\n add rsp,16'];
instruction pattern POPPI2PS32(psxmreg r)
		means [r:=(ieee32 vector ( 4 ))FLOAT((int32 vector ( 4 ))POP(mainSTACK))]
		assembles['sub esp,16\n CVTPI2PS 'r', [esp]\n SHUFPS 'r','r', 044H; move low pair to high\n'
		           ' CVTPI2PS 'r',[esp+8]\n add esp,16'];
...
instruction  pattern MOVUPI16( smaddrmode r1,xmreg rm)
        means[   rm := (i16x8)^(r1)]
    	assembles['movdqu  '    rm ', ' r1];
instruction  pattern LOPPI32(loperator op, xmregi324 rm,xmregi324 r1)
        means[  (ref i32x4) r1 :=op( (i32x4) ^( r1), (i32x4)^(  rm))]
    	assembles[ op 'ps '    r1 ',' rm';LOPPI32'];
...
instruction  pattern PORV(  xmreg rm,xmreg r1)
        means[   r1 :=OR(  ^( r1), ^(  rm))]
    	assembles[ 'por '    r1 ',' rm];
instruction  pattern PANDV  (xmreg rm,xmreg r1)
        means[   r1 :=AND(  ^( r1), ^(  rm))] 
    	assembles[ 'pand '    r1 ',' rm];
...
instruction  pattern CVTSS2SI( int32reg r1,sxmreg r0)
        means[ (ref int32) r1 :=(int32)ROUND(^(  r0))]
    	assembles['cvtss2si  '    r1 ', ' r0];
instruction  pattern CVTSS2SIr( int32reg r1,sxmreg r0)
        means[ (ref int32) r1 :=(int32)ROUND(^(  r0))]
    	assembles['cvtss2si  '    r1 ', ' r0];
...
instruction pattern rm32 means[maddrmode|int32reg];
instruction pattern MOVDl(rm32 src, lowreg dest )
	means[(ref int32)dest:=(int32) ^(src)]
	assembles['movd 'dest', dword 'src];
instruction pattern MOVDs(int32reg dest, rm32 src)
	means[dest:= (int32)^(src)]
	assembles['movd dword 'dest',  'src';MOVDs'];
...
instruction pattern CVTPI2PS(psxmreg r1,xmregi324 r0)
		means[r1:=FLOAT(^(r0))]
		assembles['cvtpi2ps 'r1','r0];
instruction  pattern CVTSI2SS( maddrmode rm,sxmreg r1)
        means[ (ref ieee32) r1 :=(ieee32)FLOAT((int32)^(  rm))]
    	assembles['cvtsi2ss  '    r1 ', ' rm];
instruction  pattern CVTIR2SS( int32reg r0,sxmreg r1)
...
instruction pattern XMPOP64(xmreg r, type t)
		means[(ref t)r:=(t)POP(mainSTACK)]
		assembles['movdqu 'r  ',[rsp]\n add rsp,16' ];
 instruction pattern XMPOP32(xmreg r, type t)
		means[(ref t)r:=(t)POP(mainSTACK)]
		assembles['movdqu 'r  ',[esp]\n add esp,16' ];
...
instruction pattern COMISS(fcondition c,sxmreg r1,sxmreg r2, breg r)
	...
	assembles['comiss 'r1 ',' r2 '\n set'c ' ' r];
instruction pattern FREP4(xmreg m, lowreg r)
	means[m:=rep(^(r),4)]
	assembles['shufps 'm ', ' r' , 0'];
...
define(XMMXcodes,ADDSS|CVTSI2SD|CVTSI2SS|CVTSD2SI|CVTIR2SS| XRLOADLIT|XRDLOADLIT| 
    CVTSS2SI|CVTSS2SIr   
	|FREP4|SSQRT|SDSQRT|OPSS|PANDNSS|PANDV|PORV|
	XPANDN| 
	OPSD|LOPPI32|OPPI32m|PORSS|PANDSS|PANDNSS|
        OPPS| OPPD|COMISSIFGOTO|  
	COMISS|OPPI8|OPPI8m|OPPI8muc|OPPI8msc|PADDSBX|PADDX|ADDPS|
	OPPI16|OPPI16m|OPPI16muc|OPPI16msc|OPPI32|SOPPI32|
	...
	ifdef(`bit64', SSRPOPI64|SSRPOP64|BVEXTEND64|POPPI2PS64|XMPOP64, 
	 SSRPOPI|SSRPOP|BVEXTEND32 |POPPI2PS32|XMPOP64)
