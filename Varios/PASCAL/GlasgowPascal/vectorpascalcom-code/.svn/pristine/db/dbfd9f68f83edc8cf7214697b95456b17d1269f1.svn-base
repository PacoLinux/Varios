...
{function clock :integer;begin clock:=round(clockspersecond*secs);end;}
function clock :double;begin clock:=secs/1000000.0;end;
begin {  main program, corresponds to procedures        } 
  ...
  begin
{	writeln('before proc5');}
    Proc5;
{	writeln('before proc 4');}
    Proc4;
      {  Char1Glob = 'A', Char2Glob = 'B', BoolGlob = false } 
{    writeln('char1glob=',char1glob,' char2glob=',char2glob);}
    Int1Glob := 2;
    Int2Glob := 3;
...
{    writeln('main for loop done');}    
 ...
 {  writeln('before proc 2');}
  ...
  writeln;
  writeln('beginclock',beginclock,' endclock',endclock);
  writeln('end secs',secs);
  ...
  if SumClocks < TooSmallTime
    ...
    {  DhrystonesPerSecond := NumberOfRuns * (ClocksPerSecond / SumClocks);}
       DhrystonesPerSecond := NumberOfRuns / SumClocks;
    {  Microseconds 	:= SumClocks * (MicrosecondsPerClock / NumberOfRuns);   }
       Microseconds 	:= SumClocks * 1000000 / NumberOfRuns;   
{  Brackets to prevent integer overflow } 

      writeln('Total Execution time in secs ', SumClocks);
