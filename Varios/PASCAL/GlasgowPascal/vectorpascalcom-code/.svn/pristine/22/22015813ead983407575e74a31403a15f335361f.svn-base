...
public class PPCCG extends PPC
{
    ...
    private int reserved_space_above_fp,
            reserved_space_below_fp, maxParamSpace = 0;
...
//		}
    ...
    {
        Node res = new Deref(regR3);
        if(t.equals("ieee32") || t.equals("ieee64"))
            res = new Deref(regF1);
        // it could be speeded up
        //if ( !t.equals(Node.int32) )     	// if statement is implement here
        /*if (t.equals(Node.ieee32)) 	res = new Deref(regsF1);
        else
        	if (t.equals(Node.ieee64))	res = new Deref(regF1);
        	else
        		if (t.equals(Node.int8) || t.equals(Node.uint8) || t.equals(Node.octet)) res = new Deref(regbR3);
        		else
        			if (t.equals(Node.int16)||t.equals(Node.uint16)||t.equals(Node.halfword)) res=new Deref(regsR3);
        			else
        				if (t.equals(Node.int32) || t.equals(Node.uint32) || t.equals(Node.word)) res = new Deref(regswR3);
        */
        ...
        if(p instanceof Unit)
            {
                ...
                w.buf.writeln(name + ":");
//			The following LINE COMMENTED ON 1-11-2010 as it look reduent
//			enterInstruction(0,p.lexLevel, w);
        ...
        if((maxParamSpace % 16) != 0)
            maxParamSpace += 16 - (maxParamSpace % 16);
        reserve=Math.abs(p.alloc.getLimit())+maxParamSpace;
        if((reserve % 16) != 0)
            reserve += 16 - (reserve % 16);
        reserve = ((reserve + 15) & (-16))+16;
        w.buf.writeln(".set frameSize" + " , " + reserve);
...
//		w.buf.writeln("leave\n ret " + (p.calleePopsArgs ? p.paramspace : 0));
    ...
    */
    ...
    public static boolean slink(String infile, String ofile)
    {
        ...
        try
            {
                // Next Line is new 14-4-2010
                String rtlname = "./mmpc/rtl.c";
                String com = "g++ -g -lm -o " + ofile + " " + infile + " " + rtlname + " " + AsmLink.libs;
    ...
    */
    public static boolean sassemble(String infile, String ofile)
    {
        ...
        try
            {
                String com = "as -a64 -o " + ofile + " " + infile;
                System.out.println(com);
                Process p = sys.exec(com);
    ...
    */
    ...
    private void enterInstruction9112010(int frameSize, int LL, Walker w)
    {
        ...
        w.buf.writeln("mr TMPREG,SP");						// Take a copy of the SP; it will be the FP of the new frame after
        ...
        // handle PmainBase
        reserve(regR21);
        w.buf.writeln("lis r21, PmainBase@ha");
        w.buf.writeln("la  r21, PmainBase@l(r21)");
                ...
                for(int i = 1; i < LL ; i++)
                    {
                        ...
                        w.buf.writeln("lwa  r29, 0(FP)");			// r29 = mem(FP)	; load the address of the nested procedure at level i
                        w.buf.writeln("stwu  r29, -4(SP)");			// mem(SP) = r29	; store that address of the nested procedure (Level i)
                    }
                w.buf.writeln("stwu TMPREG, -4(SP)");				// mem(SP) = oldSP	; store the new frame pointer
            }
...
//		w.buf.writeln("stwux SP,SP,TMPREG");
    }
    private void enterInstruction15112010(int frameSize, int LL, Walker w)
    {
        // Reg31 holds the FP & Reg1 holds the SP & Reg30 = temp (copy of the SP which will become the new frame pointer )
        // PROLOG
        // 1) Link the current frame to the new frame
        int reserved_space_above_fp = 8;
        w.buf.writeln("# Prolog");
        w.buf.writeln("mflr LR");								// Copy Link register (return address) into LR
        w.buf.writeln("stw LR,+4(SP)");				// mem(SP+4) = Return address
        w.buf.writeln("stw r31,-4(SP)");			// store the return address in the new frame
        w.buf.writeln("stw FP,-8(SP)");			// store the return address in the new frame
        w.buf.writeln("addi TMPREG,SP,-8");						// Take a copy of the SP; it will be the FP of the new frame after // handle PmainBase
        reserve(regR21);
        w.buf.writeln("lis r21, PmainBase@ha");
        w.buf.writeln("la  r21, PmainBase@l(r21)");
        // Establish the display area
...
//				w.buf.writeln("addi FP,FP,-4");				// FP = FP - 4 to move the FP to point to the place which holds
                        // a link for the nested procedure at level i
                        fp_offset -= 4;
                        reserved_space_below_fp += 4;
                        w.buf.writeln("lwa  r29,"+fp_offset+"(FP)");			// r29 = mem(FP)	; load the address of the nested procedure at level i
                        int offset = reserved_space_above_fp + reserved_space_below_fp;
                        w.buf.writeln("stw  r29,-"+offset+"(SP)");			// mem(SP) = r29	; store that address of the nested procedure (Level i)
                    }
                reserved_space_below_fp += 4;
                int offset = reserved_space_above_fp + reserved_space_below_fp;
                w.buf.writeln("stw TMPREG,-"+offset+"(SP)");				// mem(SP) = oldSP	; store the new frame pointer
            }
        w.buf.writeln("mr  FP,TMPREG");						// FP = SP		; The FP of the new frame is the SP before pushing the
        w.buf.writeln("stwu SP,-frameSize(SP)");
        //		w.buf.writeln("loadintr TMPREG,-frameSize");
    ...
    private void enterInstruction(int frameSize, int LL, Walker w)
    {
        ...
        // PROLOG
        // 1) Link the current frame to the new frame
        int reserved_space_above_fp = 4;
        w.buf.writeln("# Prolog");
        w.buf.writeln("mflr LR");								// Copy Link register (return address) into LR
        w.buf.writeln("stw LR,+4(SP)");				// mem(SP+4) = Return address
        w.buf.writeln("stw FP,0(SP)");			// store the return address in the new frame
        w.buf.writeln("mr TMPREG,SP");						// Take a copy of the SP; it will be the FP of the new frame after // handle PmainBase
        reserve(regR21);
        w.buf.writeln("lis r21, PmainBase@ha");
        w.buf.writeln("la  r21, PmainBase@l(r21)");
        // Establish the display area
    ...
    private void leave(Walker w)
    {
        ...
        w.buf.writeln("lwa  r13,0(SP)");					// load the previous frame's pointer into Reg13
        w.buf.writeln("lwa  LR,4(r13)");					// load the return address (which is the previous frame + 4) in R0
        w.buf.writeln("mtlr LR");							// move Reg0 to Link register
        w.buf.writeln("lwa FP,0(r13)");					// Update the FP to point to the previous frame
        w.buf.writeln("addi SP,r13,0");							// Update the SP so that to point to caller (previous frame) address
        //	w.buf.writeln("addi SP,SP,4");							// Update the SP so that to point to caller (previous frame) address
    ...
    boolean cgFor(Node n)	throws Exception
    {
        try
            {
                // if (match(n)) return true;
                For f = (For)n;
                Node inv = f.largestLoopInvariant();
                boolean invmemref = inv instanceof Memref;
                if(inv == null) return cgControl(n);  // no substitution possible
                if(inv instanceof Op || inv instanceof Control) return cgControl(n); // ditto
                if(temps.contains(inv.toString())) return cgControl(n);  // already substituted
                if(inv.weight() < 1 || tempcount > 1) return cgControl(n);  // it would use too many registers
                if(                             // it would not go in a register so dont substitute
                    (invmemref && (0 == (cacheLocals & optimisationLevel))))
                    return cgControl(n);
                ...
                Node newfor = ExpressionSubstituter.substituteAwithBinC(inv, substitution, n);
                Node dinv = inv;
                if(inv instanceof Memref) dinv = new Deref(inv);
                Statement opt = new Statement(
                    new Statement(
                        new Assign(temp, new Cast(t, dinv)),
                        new Statement(newfor,
                                      null
                                     )
                    )
                );
                ...
                if(verbose)
                    dlog("optimised to " + opt);
                int pre = buf.mark();
                temps.add(temp.toString());
                tempcount++;
                boolean ok = codeGen(opt);
                temps.remove(temp.toString());
                tempcount--;
                if(verbose && ok) dlog("invariant replacement ok");
                return ok;
            }
        catch(Exception e)
            {
