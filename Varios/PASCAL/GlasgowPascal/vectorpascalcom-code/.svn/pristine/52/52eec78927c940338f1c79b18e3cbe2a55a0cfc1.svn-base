...
begin { of body of main program }
   ...
   Echeck;
   if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 1');
   while not Eended do
      begin Eperm; Echeck end;

   if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 2');
   Cset:=[]; for Cv:=MinChar to MaxChar do Cray[Cv]:=false;
   Ccheck;

   if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 3');
   while not Cended do
      begin Cperm; Ccheck end;

   if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 4');
   Bset:=[]; for Bv:=false to true do Bray[Bv]:=false;
   Bcheck;

   if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 5');
   while not Bended do
      begin Bperm; Bcheck end;
if error then
      writeln(' FAIL...6.7.1-8 (CONF148) 6');
   if not error then
