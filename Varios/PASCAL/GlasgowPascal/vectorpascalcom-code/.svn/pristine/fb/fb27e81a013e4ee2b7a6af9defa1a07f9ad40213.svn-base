...
begin 
   ...
   begin  
      ...
      writeln(' FAIL...6.6.6.2-3, SIN FUNCTION (CONF135) sin(pi) gives ',sin(pi),' sin(pi/4) gives ',sin(pi/4)); 
 
   if ((-1.001<cos(pi)) and (cos(pi)<-0.999)) and 
      ((0.70<cos(pi/4)) and (cos(pi/4)<0.71)) then 
   begin
	   writeln(' PASS...6.6.6.2-3, COS FUNCTION (CONF135)'); 
      counter:=counter+1 
   end
   else 
      writeln(' FAIL...6.6.6.2-3, COS FUNCTION (CONF135) cos(pi) gives ',cos(pi), ' cos(pi/4) gives ',cos(pi/4)); 
   
   if ((2.710<exp(1)) and (exp(1)<2.720)) and 
      ((0.36<exp(-1)) and (exp(-1)<0.37)) and 
      ((8100<exp(9)) and (exp(9)<8110)) then 
   begin
	writeln(' PASS...6.6.6.2-3, EXP FUNCTION (CONF135)');
      counter:=counter+1 
   end
   else begin
