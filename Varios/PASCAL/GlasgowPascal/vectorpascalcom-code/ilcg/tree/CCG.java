...
public class CCG extends C
{
    ...
    {
        if(loct.contains("ref "))return "int";
    ...
    {
        if(!Format.isVector(ilcgtype))return ilcgtype;
                                                ...
                                                for(int i = pos+1; i <= lastloc; i++)
                                                    {
                                                        ...
                                                        String prefix ="";
                                                        if(ll instanceof Label) prefix="(int)&&";
    ...
    static void enter(int space,int lexicalLevel, Walker w,Procedure p)
    {
        space=space+16;
        int raspace=8;
        int paramcountinints = Paramspace(p) /4;
        if(p instanceof Unit)w.buf.writeln("\tint * framepointer  ;/* units have no enclosing scope so have no frampointer param*/");
        w.buf.writeln("\tchar locals["+(space+Paramspace(p)+raspace)+"];");
        w.buf.writeln("\tint    *ESP;");
        defineregs(w);
        w.buf.writeln("\tESP=((int*)&locals["+space+"]); ");
        w.buf.writeln("/* we emulate the enter instruction by storing the old EBP in the frame */");
        w.buf.writeln("\tESP[0]=(int)framepointer;");
        ...
        for(String l :list)
            {
                ...
                if(l.trim().equals(""))size=0;
                if((size %4)!=0) size=(1+(size/4))*4;
                if(size==4 && ! Format.isVector(l.trim()))
                    {
                ...
                if(size==8 && ! Format.isVector(l.trim()))
                    {
                        w.buf.writeln("\t*((int*)&(locals["+(space+raspace +pos)+"]))=("+l.trim()+ ")param"+i+";");
                    }
                if(size>=4 &&  Format.isVector(l.trim()))
                    {
                        w.buf.writeln("\tmemcpy(&(locals["+
                                      (space+raspace +pos)+"]), "+(l.trim().startsWith("ref")?"&":" ")+"param"+i+",  "+Format.lengthInBytes(l)+");"
                                     );
                    }
                i++;
                pos+=size;
            }
        //for(int i=0;i<paramcountinints;i++)w.buf.writeln("\t*((int*)&(locals["+(space+raspace +4*i)+"]))=param"+i+"; ");
        if(lexicalLevel>0)
            {
                for(i=1; i<lexicalLevel; i++)
                    {
                        w.buf.writeln("/* for lex level "+i+" fill in display */");
                        w.buf.writeln("\t\tESP[-"+i+"]=framepointer[-"+i+"]; ");
                    }
                w.buf.writeln("\tESP[-"+lexicalLevel+"]=(int)ESP; ");
            }
    ...
    */
    ...
    public static boolean scgProcedure(Procedure p, Walker w)
    {
        ...
        int mark= w.buf.mark();
        suffixes++;
        String plist=((CCG)w).outputparamlist(p,false);
        String slist=((CCG)w).outputparamlist(p,true);
        if(p.isImported)
            {
        ...
        if(p.isExported || p instanceof Unit)
            {
                ...
                ((CCG)w).predecls.writeln(w.type2Assembler(p.returntype)+" "+name+slist+";");
            }
        else
            {
                w.buf.writeln(w.type2Assembler(p.returntype)+" "+p.getStartAddressLabel() +plist);
                //   ((CCG)w).predecls.writeln(w.type2Assembler(p.returntype)+" "+p.getStartAddressLabel()+slist+";");
            }
        ;
        w.buf.writeln("{");
        p.subtree=((CCG)w).outputdatadecls(p.subtree,true);
    ...
    public static boolean slink(String infile, String ofile)
    {
        ...
        try
            {
                String com = "gcc   " + m32 + " -g   -DCCG "+(AsmLink.gcenable?"-DBOEHM":"")+"  -w -o " + ofile + " " + infile+".c "+AsmLink.rtl+" " + AsmLink.libs+ " -lm";
                System.out.println("Warning - this code generator assumes 32 bit code,\n 32 bit development system must be installed if on x86");
