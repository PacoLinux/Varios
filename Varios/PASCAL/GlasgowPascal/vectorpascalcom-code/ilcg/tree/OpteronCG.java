 ...
 */
public class OpteronCG extends Opteron
{
    ...
    public static String gcc = "gcc-4.8";
    /**
        ...
        try
            {
                ...
                if(proc==null)   // procedure variable call
                    {
                        dlog("proc var call "+proc);
                        if(decast(address) instanceof Memref)address =((Memref)decast(address)).index;
                        dlog("proc var call address="+address);
                        if(! codeGen(new Assign(regRAX, new Deref(new Memref(new Cast("int64",address),"int64")))))return false;
        ...
        if(p.toCall != null)
            {
                ...
                w.buf.writeln("cmpb $1," + name + "ready(%rip)\n jnz " + name + "init\n jmp " + name+suffix + "exit");
                w.buf.writeln(name + "init:movb $1, " + name + "ready(%rip)");
            }
        ...
        try
            {
                ...
                System.out.println("Warning code generated by -cpuOpteron is not PIC\n requires installation of gcc-4.8 for linking\n Try -cpuAMD64 instead.");
        ...
        try
            {
                ...
                System.out.println("Warning code generated by -cpuOpteron is not PIC\n requires installation of gcc-4.8 for linking\n Try -cpuAMD64 instead if using later versions of gcc.");
                System.out.println(com);
                Process p = sys.exec(com);
