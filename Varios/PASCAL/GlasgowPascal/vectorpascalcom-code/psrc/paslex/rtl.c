/*
Vector pascal run time library
 
Copyright (c) University of Glasgow 2002
Author Paul Cockshott


It contains basic i/o routines for pascal allowing
for the internal representation of characters as
Unicode
*/
#include <stdio.h>
#include <sys/time.h>
...
/** takes a pascal unicode stging and generates a asciiz string  by truncation*/
void u2asciitrunc(unsigned short *src, char * dest)
{	int i ;

        for(i=0;i<=((*src)&255);i++)dest[i]=(src[i]&127);
	dest[((*src)&255)+1]=0;
}
...
{  FILE *f;checkerr(*filenum);f=files[*filenum].f; return lseek(f,pos,mode);}
...
{
	checkerr(*filenum);
	files[*filenum].f=fopen(files[*filenum].name,"wb");
	files[*filenum].status=OPEN;
}
void reset(int *filenum)
{
	checkerr(*filenum);
	files[*filenum].f=fopen(files[*filenum].name,"rb+");
...
void readcharnarrow(int fn,char *c)/* reads an ascii char */
{
	FILE *f;checkerr(fn);
	f=files[fn].f;
...
//int stringcompare(char *s1,char *s2){return cstringcompare(s1,s2);}	
...
void printint64(int fn,long x,int width,int afterpoint){
...
char * getargv(int i) { return Argv[i];}
