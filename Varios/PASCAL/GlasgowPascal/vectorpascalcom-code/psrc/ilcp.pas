...
type 
    ...
    printsas:string[40];
     ...
     ilcgnode= record
                ...
                ifnode:( condition,
                         ...
                         action,
                         alternative:^ilcgnode;);

                unboundformat,
                format:(formatarg:integer);
                monop,
                extlabel,
                dyadicop:(opname:string[20];);
                labelnode,
                param,
        	reg:(index:integer;);
        	sequence:(current,next:^ilcgnode;);
        	forloop:( indexvar,
    ...
    pilcgnode=^ilcgnode;
     ...
     printsort = (printcharacter,printparam);
     printnode = record

                 next:^printnode;

               case literal:printsort of
               printcharacter:(litval:char;);
     ...
     nodevisitor=procedure (var p:pilcgnode) is nested;
...
var
	...
	function findfreereg(format:integer):integer;
     ...
     function new_intlit(i:double;intformat:integer):pilcgnode;
     ...
     function new_regstack(stack:RegisterStack):pilcgnode;
     ...
     procedure visit(var p:pilcgnode;visitor:nodevisitor{procedure(var pc:pilcgnode)});
...
implementation
    ...
    type
     ...
     var bitbucket:string ; { this string is used to dispose of matches we }
      ...
      function binarymatch(src,template:pilcgnode):boolean;    forward;
      procedure pappends(var l:pprintlist; s:string);
       procedure recurse(var l2:pprintlist);
       begin
            if l2=nil
            then l2:=string2printlist(s)
            else recurse(l2^.next);
       end;
      begin
          recurse(l);
      end;
     function ii2i(ii:intint):integer;begin ii2i:=round(1.0*ii) end;
     ...
     { numeric conversion                                       }
      ...
      type rstring=string[24];
      Function real_2_string(A:real): rstring;
     ...
     procedure visit(var p:pilcgnode;visitor:nodevisitor{procedure(var pc:pilcgnode)});
       procedure vs (var pc:pilcgnode);begin visitor(pc);end;
     begin
    ...
    procedurenode: visit(procedurebody, @ vs );

                gotonode: visit(dest, @ vs );
                ...
                forloop:begin visit(indexvar, @ vs );
                              visit(start, @ vs );
                              visit(stop, @ vs );
                              visit(incr, @ vs );
                              visit(loopaction, @ vs );
        ...
        begin
	         ...
	         with p^ do
	          case tag of
	                  ...
	                  monad:   begin
	                                previsit(fn,visitor);
	                                previsit(arg,visitor);
	                           end;
	                  constant,
	                  ref,
	                  memref,
					  deref:   previsit(arg,visitor);
	
	                        procedurenode: previsit(procedurebody,visitor);
	
	                  gotonode: previsit(dest,visitor);
	
	                  dyad:   begin
	                           previsit(arg,visitor);
	                           previsit(fn,visitor);
	                           previsit(arg2,visitor);
	                          end;
	
	                  location: previsit(locvalue,visitor);
	
	                        forloop:begin previsit(indexvar,visitor);
	                              previsit(start,visitor);
	                              previsit(stop,visitor);
	                              previsit(incr,visitor);
	                              previsit(loopaction,visitor);
	                        end;
	                  typecast: begin
	                             previsit(arg,visitor);
	                             previsit(arg2,visitor);
	                  ...
	                  arraysubscript: begin
	                                       previsit(base,visitor);
	                                       previsit(offset,visitor);
	                                  end;
	
	                  assignop:begin
	                                previsit(dest,visitor);
	                                previsit(src,visitor);
	                           end     ;
	                  sequence:begin
	                                previsit(current,visitor);
	                                previsit(next,visitor);
	                           end;
	                  ifnode:begin
	
	                              previsit(condition,visitor);
	                              previsit(action,visitor);
	                              previsit(alternative,visitor);
	                         end;
	              else write( ' visit unknown node type tag ',ord(tag))
	              end;
				end;
    ...
    deref:   modify(arg,visitor);
     ...
     begin
          if p<>nil
             then 
             if p^.tag=assignop                   then
             begin
					p2:= decast(p^.dest);
                       if p2^.tag<> arraysubscript
...
function findfreereg(format:integer):integer;
    ...
    begin new_extlabel:= new_register(fp)
     ...
     function fd(p:pilcgnode):pilcgnode;{forcederef}
     begin
       if  (fref and getformat(p))<>0 then
         fd:=new_deref(p)
       else
         fd:=p;
     end;
     ...
     type
         ...
         (*! This attempts to match n against p, returns result in s *)
           ...
           *)
           ...
           label 1; var i,d:integer;  ok:boolean;
           ...
           *)
           ...
           var tempnode,t2:pilcgnode; i:integer; ok:boolean;
               saved:evalcontext;
               parampat:pattern;
           begin
      ...
      gotonode:   if src^.tag<>gotonode
                  ...
                  constant:   begin
                                   ...
                                   then begin
                                      ...
                                      then begin
                                        ...
                                        then begin
                                             binarymatch:=true;
                                             paramresults[i] := int2str(tempnode^.intlitarg);
                                            { str(tempnode^.intlitarg,paramresults[i])}
                                        end
                                        else if tempnode^.tag=reallit
                                        then begin
                                             binarymatch:=true;
                                             paramresults[i] := real_2_string(tempnode^.reallitarg);
         ...
         begin    { patternmatch }
            ...
            recursiondepth:=recursiondepth+1;
             if verbose
                   then begin
                        for z:=0 to recursiondepth do write(space);
          ...
          with m^ do begin
            ...
            procedurenode:
                ...
                reg: t1:=false; { not allowed at the top level }
                   ...
                   if verbose
                   then begin 
                           writeln('could not match node of type ',ord(tag));
						 {  printtree(m,output);}
					end;
                   t1:=false;
                 end
           ...
           begin
                ...
                then binarymatch := template=nil
                 ...
                 then binarymatch:=false
                  ...
                  deref: begin
							  t2:=decast (template^.arg);
                              if t2^.tag=param then  
                              { we have a candidate for  filling a register   }
                              
                              begin
                               if src^.tag=deref  then 
                               if binarymatch(src^.arg,template^.arg)then binarymatch:=true
                               else if binarymatch(src,decast(template^.arg))  then binarymatch:=true
                               else if src^.tag<>deref then binarymatch:=false
                               else binarymatch:=binarymatch(src^.arg,template^.arg);
                              end
                              else  if src^.tag<>deref
                              then binarymatch:=false
     ...
     procedure printassmspec(p:pprintlist;var f:text);
           ...
           *)
           begin
                ...
                then begin
                   case p^.literal of
                    printcharacter: write(f,p^.litval)      ;
